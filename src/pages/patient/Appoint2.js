...
import { dumpDoctors, dumpPeriods, dumpAppointed } from '../../dummyData';
...
class Appoint2 extends Component {
  constructor(props) {
    super(props);
    this.state = {
      chosenDoc: false,
    ...
    this.allDoctors = [];
    this.getDoctorsAndPeriods = this.getDoctorsAndPeriods.bind(this);
  ...
  componentWillMount() {
    this.getDoctorsAndPeriods();
  }

  getDoctorsAndPeriods() {
    // Get doctor
    const doctors = dumpDoctors();
    let chosenDoc = false;
    for (let i = 0; i < doctors.length; i += 1) {
      if (doctors[i].hospitalID === this.props.params.id) {
        chosenDoc = doctors[i];
      }
    }
    ...
    if (!chosenDoc) {
      ...
      this.setState({ chosenDoc, periods, appointedDates });
    }
  }
...
export default withRouter(Appoint2);
